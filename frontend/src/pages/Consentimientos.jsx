...
function tipoConsentimientoLabel(tipo) {
  return (
    TIPOS_CONSENTIMIENTO.find((item) => item.value === tipo)?.label || tipo
  );
}
    ...
    <div
      className={[
        ...
        anulado
          ...
          : 'border-teal-border hover:border-teal/50',
      ].join(' ')}
    >
          ...
          <div className="min-w-0">
            <p
              className={[
                ...
                anulado ? 'text-status-red' : 'text-primary',
              ].join(' ')}
            >
              {title}
            </p>

            <p className="text-[11px] text-teal-muted mt-0.5 truncate">
              {subtitle}
            </p>
...
export default function Consentimientos() {
  ...
  const pacientesFiltrados = useMemo(() => {
    ...
    return pacientes.filter((paciente) => {
      ...
      return (
        ...
        String(paciente.numero_documento || '').includes(texto)
      );
    });
  }, [pacientes, busqueda]);
  ...
  async function cargarDocumentos(id = pacienteId) {
    ...
    try {
      ...
      setErrorDocs(
        err.error || 'No se pudieron cargar los documentos del paciente.'
      );
    } finally {
    ...
    try {
      await api.crearConsentimiento({
        ...
        campos_especificos: consentimientoForm.detalle
          ...
          : null,
        nombre_paciente_declarado:
          consentimientoForm.nombre_paciente_declarado || null,
      ...
      setErrorDocs(
        err.error || 'No se pudo crear el consentimiento.'
      );
    } finally {
      setGuardando(null);
    }
  ...
  async function crearCertificado(event) {
    ...
    try {
      ...
      setErrorDocs(
        err.error || 'No se pudo crear el certificado.'
      );
    } finally {
      setGuardando(null);
    }
  ...
  async function eliminarDocumento(tipo, id) {
    ...
    try {
      ...
      setErrorDocs(
        err.error || 'No se pudo eliminar el documento.'
      );
    } finally {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: consentimientos.filter(
        (c) =>
          !estaAnulado(c) &&
          (c.firma_paciente || c.firma_doctor)
      ).length,
      sub: 'consentimientos',
      accentColor: '#85B7EB',
    },
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        {cargandoDocs ? (
                          ...
                          consentimientos.map((consentimiento) => {
                            ...
                            const anulado =
                              estaAnulado(consentimiento);

                            return (
                              <DocumentoCard
                                key={consentimiento.id}
                                icon={FileText}
                                title={tipoConsentimientoLabel(
                                  consentimiento.tipo
                                )}
                                subtitle={
                                  consentimiento.ciudad ||
                                  'Villavicencio'
                                }
                                fecha={formatoFecha(
                                ...
                                anulado={anulado}
                                motivoAnulacion={
                                  consentimiento.motivo_anulacion
                                }
                                puedeEliminarDocumento={puedeEliminar(
                                  consentimiento
                                )}
                                procesando={
                                  procesandoDoc ===
