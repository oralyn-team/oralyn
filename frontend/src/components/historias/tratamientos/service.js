...
export function buildCotizacionPayload(data, pacienteId) {
  ...
  return {
    ...
    procedimientos: procedimientos.map((p, index) => ({
      id: (typeof p.id === 'number' && Number.isInteger(p.id)) ? p.id : (typeof p.id === 'string' && /^\d+$/.test(p.id) ? Number(p.id) : undefined),
      ...
      valor_unitario: Number(p.valorUnitario ?? p.valor_unitario),
