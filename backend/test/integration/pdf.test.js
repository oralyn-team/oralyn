...
// SKIP: paciente.hbs fue código de prueba, nunca se usó en producción
// ni se implementó en el frontend. Pendiente de decisión: implementar
// la plantilla o eliminar el endpoint /pdf/paciente definitivamente.
test.skip('PDF Paciente: Genera PDF correctamente con cabeceras y estructura válidas', async (t) => {
  const harness = await startAppWithPrisma(createPDFPrismaMock())
  t.after(() => harness.close())
...
test.skip('PDF Paciente: Retorna 404 si el paciente no existe', async (t) => {
  const harness = await startAppWithPrisma(createPDFPrismaMock())
  t.after(() => harness.close())
...
test.skip('PDF Paciente: Retorna 404 si el paciente pertenece a otro consultorio (Permisos)', async (t) => {
  const harness = await startAppWithPrisma(createPDFPrismaMock())
  t.after(() => harness.close())
